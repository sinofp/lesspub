...
async function like(incoming) {
  ...
  var id = $$Object.StringOption.classify(Caml_option.valFromOption(match$1));
  if (id.TAG !== /* String */0) {
    return {
            statusCode: 501,
            body: "Give me only the object id, please"
          };
  }
  var slug = id._0.slice(noteBaseLength);
  var forMe = await slugExist(slug);
  var path = slug + "-likes.jsonld";
  if (!forMe || await Fetch.GitHub.insertToFile(match, path)) {
    return {
            statusCode: 200
          };
  } else {
